...
import { CategoryInfoHealthItems } from '@/src/types/models'
import { CategoryInfoHealthItemsEnum } from '@/src/enums/categories'
import SpecificationsItem from '@/src/components/atoms/SpecificationsItem'

type Props = {
  categoryInfo: CategoryInfoHealthItems
}

const HealthItemsSpecifications = ({ categoryInfo }: Props) => {
  const { palette } = useTheme()

  const { t } = useTranslation('ad')

  const array = [{ field: CategoryInfoHealthItemsEnum.Subcategory, type: SpecificationItemType.Title }]
...
export default HealthItemsSpecifications
