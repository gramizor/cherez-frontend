...
import { CategoryInfoDevices } from '@/src/types/models'
import { CategoryInfoDevicesEnum } from '@/src/enums/categories'
import SpecificationsItem from '@/src/components/atoms/SpecificationsItem'

type Props = {
  categoryInfo: CategoryInfoDevices
}

const DevicesSpecifications = ({ categoryInfo }: Props) => {
  const { palette } = useTheme()

  const { t } = useTranslation('ad')

  const array = [
    { field: CategoryInfoDevicesEnum.Subcategory, type: SpecificationItemType.Title },
    { field: CategoryInfoDevicesEnum.Quality, type: SpecificationItemType.Title },
  ]
