...
import { CategoryInfoHousehold } from '@/src/types/models'
import { CategoryInfoHouseholdEnum } from '@/src/enums/categories'
import SpecificationsItem from '@/src/components/atoms/SpecificationsItem'

type Props = {
  categoryInfo: CategoryInfoHousehold
}

const HouseholdSpecifications = ({ categoryInfo }: Props) => {
  const { palette } = useTheme()

  const { t } = useTranslation('ad')

  const array = [{ field: CategoryInfoHouseholdEnum.Subcategory, type: SpecificationItemType.Title }]
...
export default HouseholdSpecifications
