...
import { CategoryInfoSpareParts } from '@/src/types/models'
import { CategoryInfoSparePartsEnum } from '@/src/enums/categories'
import SpecificationsItem from '@/src/components/atoms/SpecificationsItem'

type Props = {
  categoryInfo: CategoryInfoSpareParts
}

const SparePartsSpecifications = ({ categoryInfo }: Props) => {
  const { palette } = useTheme()

  const { t } = useTranslation('ad')

  const array = [{ field: CategoryInfoSparePartsEnum.Subcategory, type: SpecificationItemType.Title }]
...
export default SparePartsSpecifications
