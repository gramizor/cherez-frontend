...
import { CategoryInfoAnimals } from '@/src/types/models'
import { CategoryInfoAnimalsEnum } from '@/src/enums/categories'
import SpecificationsItem from '@/src/components/atoms/SpecificationsItem'

type Props = {
  categoryInfo: CategoryInfoAnimals
}

const AnimalsSpecifications = ({ categoryInfo }: Props) => {
  const { palette } = useTheme()

  const { t } = useTranslation('ad')

  const array = [{ field: CategoryInfoAnimalsEnum.AdType, type: SpecificationItemType.Title }]
...
export default AnimalsSpecifications
